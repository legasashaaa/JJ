...
class TelegramSpyBot:
    ...
    async def handle_callback_query(self, update: Dict):
        ...
        try:
            ...
            if action == "user_info":
                ...
                user_id = int(parts[1])
                await self.track_user_friends(chat_id, user_id)
                ...
                user_id = int(parts[1])
                await self.show_recent_replies(chat_id, user_id)
        ...
        try:
            ...
            keyboard_buttons = [
                ...
                [
                    ...
                    {"text": "👥 Кто отвечает", "callback_data": f"show_replies:{user_id}"}
                ],
                [
    ...
    async def show_recent_replies(self, chat_id: int, user_id: int):
        """Показывает недавние ответы на сообщения пользователя"""
        try:
            await self.send_bot_message(chat_id, "🔍 Ищу ответы на сообщения пользователя...")
            ...
            # Проверяем каждый чат за последние 24 часа
            ...
            for i, chat_identifier in enumerate(chat_identifiers, 1):
                try:
                    if i % 2 == 0:  # Прогресс каждые 2 чата
                        progress_msg = (
                            f"🔍 Прогресс: {i}/{len(chat_identifiers)} чатов\n"
                            f"📊 Найдено ответов: {len(replies_found)}\n"
                            f"⏳ Продолжаю поиск..."
                        )
                        await self.send_bot_message(chat_id, progress_msg)
                    ...
                    # Получаем сообщения пользователя за последние 24 часа
                    user_messages = []
                    async for message in self.client.iter_messages(
                        chat,
                        limit=50,
                    ...
                    for user_msg in user_messages:
                        try:
                            # Получаем сообщения после этого
                                ...
                                if (reply and reply.reply_to and 
                                    ...
                                    try:
                                        ...
                                        replies_found.append({
                                            ...
                                            "replier": sender_name or "Неизвестный",
                                        ...
                                        if len(replies_found) <= 3:
                                            reply_info = (
                                                f"💬 <b>Найден ответ:</b>\n\n"
                                                f"👤 Кому ответили: {user.first_name}\n"
                                                f"👥 Кто ответил: {sender_name or 'Неизвестный'}\n"
                                                f"💬 Чат: {chat_name[:50]}\n"
                    ...
                    print(f"Ошибка поиска ответов в чате {chat_identifier}: {e}")
                    continue
            ...
            if replies_found:
                total_text = (
                    ...
                    f"👤 Пользователь: {user.first_name if hasattr(user, 'first_name') else 'ID: ' + str(user_id)}\n"
                    f"📊 Найдено ответов: {len(replies_found)}\n"
                    f"📁 Проверено чатов: {checked_chats} из {len(chat_identifiers)}\n"
                ...
                if len(replies_found) > 3:
                    remaining = replies_found[3:min(8, len(replies_found))]
                    for reply in remaining:
                        reply_info = (
                            f"💬 <b>Еще ответов:</b>\n\n"
                            f"👥 Кто ответил: {reply['replier']}\n"
                            f"💬 Чат: {reply['chat'][:50]}\n"
                    ...
                    if len(replies_found) > 8:
                        await self.send_bot_message(chat_id,
                            f"📄 <b>И еще {len(replies_found) - 8} ответов...</b>\n"
                            f"Всего найдено: {len(replies_found)}"
                        )
            else:
                total_text = (
                    ...
                    f"📁 Проверено чатов: {checked_chats} из {len(chat_identifiers)}\n"
                    f"⏳ Период: последние 24 часа\n\n"
                    f"<i>За последние 24 часа никто не отвечал на сообщения пользователя</i>"
                )
            
            keyboard = self.create_keyboard([
                [
                    {"text": f"👁 Следить за ответами", "callback_data": f"monitor_replies:{user_id}"},
                    {"text": "🔍 Найти сообщения", "callback_data": f"search_messages:{user_id}"}
                ],
                [
                    {"text": "📊 Профиль", "callback_data": f"user_info:{user_id}"},
            ...
            print(f"Ошибка поиска ответов: {e}")
            await self.send_bot_message(chat_id, f"❌ Ошибка поиска: {str(e)[:100]}")
    
    async def track_user_friends(self, chat_id: int, user_id: int):
        """Отслеживает друзей пользователя"""
        try:
            friends = await self.get_user_statistics(user_id)
            
            if friends['friends']:
                friends_text = "👥 <b>Частые собеседники:</b>\n\n"
                for i, friend in enumerate(friends['friends'][:10], 1):
                    friends_text += f"{i}. {friend}\n"
                
                friends_text += f"\n📊 Всего: {len(friends['friends'])} человек"
            else:
                friends_text = "❌ Частых собеседников не обнаружено"
            
            # Добавляем информацию об активности
            friends_text += f"\n\n📈 <b>Активность:</b> {friends['activity_level']}"
            friends_text += f"\n👥 <b>Общих чатов:</b> {friends['common_chats']}"
            ...
            await self.send_bot_message(chat_id, friends_text, keyboard)
            
        except Exception as e:
            print(f"Ошибка отслеживания друзей: {e}")
            await self.send_bot_message(chat_id, f"❌ Ошибка: {str(e)[:100]}")
    ...
    async def run(self):
        ...
        print("="*60)
        print("🤖 TELEGRAM SPY BOT v3.1")
        print("="*60)
        print("✨ Новая функция: Отслеживание ответов на сообщения")
        print("="*60)
        ...
        test_msg = (
            ...
            f"✨ <b>Новая функция:</b> Отслеживание ответов на сообщения\n\n"
