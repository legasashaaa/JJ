...
class TelegramSpyBot:
    ...
    async def show_replies_to_user(self, chat_id: int, user_id: int, page: int = 0):
        ...
        try:
            ...
            keyboard_buttons.append([
                ...
                {"text": "🔄 Обновить", "callback_data": f"replies_to_user:{user_id}:0"}
            ])
    ...
    async def show_replies_from_user(self, chat_id: int, user_id: int, page: int = 0):
        ...
        try:
            ...
            keyboard_buttons.append([
                {"text": "🔙 Назад к меню", "callback_data": f"show_replies:{user_id}"},
                {"text": "🔄 Обновить", "callback_data": f"replies_from_user:{user_id}:0"}
            ])
    ...
    async def show_reply_pair_details(self, chat_id: int, user_id: int, index: int, direction: str):
        ...
        try:
            ...
            await self.send_reply_page(chat_id, user_id, target_replies, 0, total_pages, title, direction, index)
            
        except Exception as e:
            print(f"Ошибка показа деталей реплаев: {e}")
            await self.send_bot_message(chat_id, f"❌ Ошибка: {str(e)[:100]}")
    
    async def send_reply_page(self, chat_id: int, user_id: int, replies: List[Dict], page: int, total_pages: int, 
                             title: str, direction: str, user_index: int):
        """Отправляет страницу с реплаями"""
        try:
            items_per_page = 3
            start_idx = page * items_per_page
            end_idx = min((page + 1) * items_per_page, len(replies))
            
            message_text = f"💬 <b>{title}</b>\n\n"
            message_text += f"📊 Всего ответов: {len(replies)}\n"
    ...
    async def run(self):
        ...
        print("="*60)
        print("🤖 TELEGRAM SPY BOT v3.2")
        print("="*60)
        print("✨ Новая функция: Анализ ответов с пагинацией")
        print("="*60)
        ...
        test_msg = (
            ...
            f"✨ <b>Новые функции:</b>\n"
            f"• Анализ кто отвечает пользователю\n"
            f"• Анализ кому отвечает пользователь\n"
