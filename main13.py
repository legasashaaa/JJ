...
class TelegramSpyBot:
        ...
        try:
            ...
            if text.startswith("/start"):
                welcome_msg = (
                    ...
                    "⚡ <b>Улучшенная версия:</b>\n"
                    "• Ускоренный поиск (до 10 чатов одновременно)\n"
                    "• Убраны все лимиты на сообщения\n"
                    "• Исправлен поиск реплаев\n"
                    "• Поддержка ссылок t.me/+invite_code\n"
                    "• Улучшенный поиск по ID\n"
                    "• Быстрый поиск реплаев\n\n"
                    "📝 <b>Использование:</b>\n"
                ...
                help_msg = (
                    ...
                    "⚡ <b>Улучшенная версия:</b>\n"
                    "• Поиск без лимитов\n"
                    "• Обработка 10 чатов одновременно\n"
                    "• Быстрый поиск реплаев\n"
                    "• Поддержка всех форматов ссылок\n\n"
                    "👁 <b>Отслеживание:</b>\n"
    ...
    async def handle_user_search(self, chat_id: int, user_input: str):
        ...
        try:
            ...
            if user_input_clean.isdigit() or (user_input_clean.startswith('-') and user_input_clean[1:].isdigit()):
                ...
                try:
                    # Пробуем разные методы поиска по ID
                    ...
                    try:
                        ...
                        try:
                            # Пробуем через InputPeerUser
                            user = await self.client.get_entity(InputPeerUser(user_id=user_id, access_hash=0))
                        except:
                            try:
                                # Последняя попытка - через resolve_username
                                user = await self.client(ResolveUsernameRequest(username=str(user_id)))
                            except:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            user = await self.client(ResolveUsernameRequest(username=username))
                        except:
            ...
            for dialog in dialogs:
                try:
                    ...
                    if dialog.is_group or dialog.is_channel:
                        ...
                        try:
                            # Быстрая проверка через поиск сообщений
                            total_messages = 0
                            async for _ in self.client.iter_messages(
                                dialog.entity,
                                from_user=user,
                                limit=50  # Только 50 для быстрой проверки
                            ):
                                total_messages += 1
    ...
    async def show_message_count(self, chat_id: int, user_id: int):
        ...
        try:
            ...
            chat_groups = [chats[i:i+10] for i in range(0, len(chats), 10)]
            
            for group_idx, chat_group in enumerate(chat_groups):
                # Обрабатываем группу чатов параллельно
                tasks = []
                for chat_identifier in chat_group:
                    tasks.append(self.count_messages_in_chat(chat_identifier, user))
                ...
                for result in results:
                    ...
                    chat_name, message_count = result
                    if message_count > 0:
    ...
    async def count_messages_in_chat(self, chat_identifier: str, user):
        ...
        try:
            ...
            # Подсчет сообщений БЕЗ ЛИМИТА
            message_count = 0
            try:
                async for _ in self.client.iter_messages(
                    chat,
                    from_user=user
                    # Без limit - получаем все сообщения
                ):
                    message_count += 1
            except Exception as e:
    ...
    async def search_user_messages(self, chat_id: int, user_id: int, search_text: str):
        ...
        try:
            ...
            for group_idx, chat_group in enumerate(chat_groups):
                ...
                tasks = []
                for chat_identifier in chat_group:
                    tasks.append(self.search_in_chat(chat_identifier, user, search_text))
                ...
                # Отправляем найденные сообщения (первые 5 сразу)
    ...
    async def search_in_chat(self, chat_identifier: str, user, search_text: str):
        ...
        try:
            ...
            while self.tracking_status.get(user_id, {}).get('replies', False):
                try:
                    ...
                    # Разбиваем чаты на группы по 10 для параллельной обработки
                    chat_groups = [chat_identifiers[i:i+10] for i in range(0, len(chat_identifiers), 10)]
                    
                    for chat_group in chat_groups:
                        # Обрабатываем группу чатов параллельно
                        tasks = []
                        for chat_identifier in chat_group:
                            tasks.append(self.check_replies_in_chat(chat_identifier, user, last_check))
                        ...
                        for result in results:
                            ...
                            for reply_info in result:
                                # Отправляем уведомление о найденном ответе
                                notification = (
    ...
    async def check_replies_in_chat(self, chat_identifier: str, user, last_check: datetime):
        ...
        try:
            ...
            async for message in self.client.iter_messages(
                ...
                try:
                    # Используем search_messages для поиска реплаев
                    ...
                    for reply in replies:
                        if reply and hasattr(reply, 'from_id') and reply.from_id:
                            try:
                    ...
                    print(f"Ошибка проверки ответов: {e}")
                    continue
    ...
    async def search_replies_to_user(self, chat_id: int, user_id: int, target_user_input: str):
        ...
        try:
            ...
            if target_user_input_clean.isdigit() or (target_user_input_clean.startswith('-') and target_user_input_clean[1:].isdigit()):
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            target_user = await self.client.get_entity(InputPeerUser(user_id=target_user_id, access_hash=0))
                        except:
            ...
            found_replies = []
            checked_chats = 0
            start_date = datetime.now() - timedelta(days=180)  # Последние 180 дней
            ...
            for group_idx, chat_group in enumerate(chat_groups):
                ...
                tasks = []
                for chat_identifier in chat_group:
                    tasks.append(self.search_replies_to_in_chat(chat_identifier, user, target_user, start_date))
                ...
                # Отправляем найденные ответы (первые 3 сразу)
            ...
            if found_replies:
                ...
                total_text = (
                    ...
                    f"📁 Проверено чатов: {checked_chats} из {len(chats)}\n"
                    f"⏳ Период: последние 180 дней\n\n"
                    f"<i>Пользователь {target_name} не отвечал на сообщения нашего пользователя за последние 180 дней</i>"
                )
    ...
    async def search_replies_to_in_chat(self, chat_identifier: str, user, target_user, start_date: datetime):
        ...
        try:
            ...
            # Получаем сообщения нашего пользователя за последние 180 дней
            user_messages = []
            async for message in self.client.iter_messages(
                chat,
                from_user=user,
            ...
            # Для каждого сообщения пользователя ищем ответы от целевого пользователя
            for user_msg in user_messages:
                try:
                    # Ищем ответы на это сообщение
                    # Используем search_messages с фильтром по отправителю
                    async for reply in self.client.iter_messages(
                        ...
                        if reply and reply.reply_to and reply.reply_to.reply_to_msg_id == user_msg.id:
    ...
    async def search_replies_from_user(self, chat_id: int, user_id: int, target_user_input: str):
        ...
        try:
            ...
            for group_idx, chat_group in enumerate(chat_groups):
                ...
                tasks = []
                for chat_identifier in chat_group:
                    tasks.append(self.search_replies_from_in_chat(chat_identifier, user, target_user, start_date))
                ...
                if len(found_replies) <= 3:
                    for reply in found_replies:
                        ...
                        reply_info = (
                            f"💬 <b>Найден ответ нашему пользователя {target_name}:</b>\n\n"
                            f"👤 Кому отвечал: {target_name}\n"
                            f"💬 Чат: {reply['chat'][:50]}\n"
                            f"📅 Время: {reply['reply_time']}\n"
                            f"📝 Оригинал: {reply['original_text']}\n"
                            f"📝 Ответ: {reply['reply_text']}\n"
            ...
            if found_replies:
                ...
                total_text = (
                    ...
                    f"📁 Проверено чатов: {checked_chats} из {len(chats)}\n"
                    f"⏳ Период: последние 180 дней\n\n"
                    f"<i>Наш пользователь не отвечал на сообщения пользователя {target_name} за последние 180 дней</i>"
                )
    ...
    async def search_replies_from_in_chat(self, chat_identifier: str, user, target_user, start_date: datetime):
    ...
    async def collect_replies_data(self, user_id: int):
        ...
        try:
            ...
            for chat_group in chat_groups:
                ...
                for chat_identifier in chat_group:
                    tasks_to.append(self.collect_replies_to_in_chat(chat_identifier, user, start_date))
                    tasks_from.append(self.collect_replies_from_in_chat(chat_identifier, user, start_date))
    ...
    async def collect_replies_to_in_chat(self, chat_identifier: str, user, start_date: datetime):
        """Собирает данные о том, кто отвечает пользователю в конкретном чате"""
        replies = []
        try:
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
    ...
    async def collect_replies_from_in_chat(self, chat_identifier: str, user, start_date: datetime):
        """Собирает данные о том, кому отвечает пользователь в конкретном чате"""
        replies = []
        try:
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
        ...
        try:
            ...
            if not replies_to_user:
                await self.send_bot_message(chat_id,
                    ...
                    f"👤 Пользователь: <code>{user_id}</code>\n"
                    f"⏳ Период: последние 180 дней\n\n"
                    f"За последние 180 дней никто не отвечал на сообщения пользователя."
                )
                return
        ...
        try:
            ...
            if not replies_from_user:
                await self.send_bot_message(chat_id,
                    ...
                    f"👤 Пользователь: <code>{user_id}</code>\n"
                    f"⏳ Период: последние 180 дней\n\n"
                    f"За последние 180 дней пользователь никому не отвечал."
                )
                return
