...
class UserProfile:
    ...
    user_chats: List[Dict] = None       # Список чатов, в которых состоит пользователь
...
class TelegramSpyBot:
    ...
    async def handle_callback_query(self, update: Dict):
        ...
        try:
            ...
            elif action == "find_user_chats":
                user_id = int(parts[1])
                await self.find_user_chats(chat_id, user_id)
    ...
    async def handle_user_search(self, chat_id: int, user_input: str):
        ...
        try:
            ...
            # Проверяем есть ли уже информация о чатах пользователя
            user_chats = []
            if user.id in self.monitored_users:
                user_chats = self.monitored_users[user.id].user_chats
    ...
    async def show_user_actions(self, chat_id: int, user_id: int):
        ...
        try:
            ...
            # Добавляем информацию о чатах, если они уже найдены
            if profile.user_chats:
                user_info += f"💬 <b>Чаты пользователя ({len(profile.user_chats)}):</b>\n"
                for i, chat in enumerate(profile.user_chats[:5], 1):
                    chat_name = chat.get('title', chat.get('username', f'Чат {chat.get("id", "N/A")}'))[:30]
                    chat_type = "группа" if chat.get('is_group') else "канал" if chat.get('is_channel') else "личный"
                    user_info += f"{i}. {chat_name} ({chat_type})\n"
                
                if len(profile.user_chats) > 5:
                    user_info += f"... и еще {len(profile.user_chats) - 5} чатов\n"
                
                user_info += "\n"
            
            user_info += "<i>Выберите действие:</i>"
            ...
            keyboard_buttons = [
                ...
                [
                    ...
                    {"text": "💬 Найти чаты пользователя", "callback_data": f"find_user_chats:{user_id}"}
                ],
                [
    ...
    async def find_user_chats(self, chat_id: int, user_id: int):
        """Ищет чаты, в которых состоит пользователь"""
        try:
            await self.send_bot_message(chat_id, 
                f"🔍 Ищу чаты, в которых состоит пользователь...\n"
            ...
            for dialog in dialogs:
                try:
                    chat_info = {
                        "id": dialog.id,
                        "title": dialog.name[:50] if dialog.name else f"Чат {dialog.id}",
                    ...
                    if dialog.is_group or dialog.is_channel:
                        try:
                            chat = await self.client.get_entity(dialog.entity)
                            if hasattr(chat, 'username') and chat.username:
                                chat_info["username"] = chat.username
                            
                            # Проверяем участников (для групп)
                            if dialog.is_group:
                                try:
                                    ...
                                    if user.id in participant_ids:
                                        ...
                                        print(f"✅ Пользователь найден в группе: {dialog.name}")
                                except Exception as e:
                                    # Если не удалось получить участников, все равно добавляем чат
                                    user_chats.append(chat_info)
                                    print(f"⚠️ Добавлен чат без проверки участников: {dialog.name}")
                            elif dialog.is_channel:
                                # Для каналов добавляем все
                                user_chats.append(chat_info)
                                print(f"✅ Добавлен канал: {dialog.name}")
                                
                        except Exception as e:
                            print(f"Ошибка получения информации о чате {dialog.name}: {e}")
                            continue
                    
                    elif dialog.is_user:
                        # Личные чаты
                        if dialog.entity.id == user.id:
                            user_chats.append(chat_info)
                            print(f"✅ Личный чат: {dialog.name}")
            ...
            # Сохраняем информацию о чатах в профиль
            if user_id in self.monitored_users:
                self.monitored_users[user_id].user_chats = user_chats
                self.save_monitored_users()
            
            # Формируем отчет
            if user_chats:
                report_text = (
                    f"✅ <b>Найдены чаты пользователя!</b>\n\n"
                    f"👤 Пользователь: {user.first_name if hasattr(user, 'first_name') else 'ID: ' + str(user_id)}\n"
                    f"💬 Всего чатов: {len(user_chats)}\n\n"
                )
                
                # Группируем чаты по типам
                groups = [c for c in user_chats if c.get('is_group')]
                channels = [c for c in user_chats if c.get('is_channel')]
                private = [c for c in user_chats if c.get('is_user')]
                
                if groups:
                    report_text += f"👥 <b>Группы ({len(groups)}):</b>\n"
                    for i, chat in enumerate(groups[:10], 1):
                        username_str = f" @{chat['username']}" if chat.get('username') else ""
                        report_text += f"{i}. {chat['title']}{username_str}\n"
                    
                    if len(groups) > 10:
                        report_text += f"... и еще {len(groups) - 10} групп\n"
                    report_text += "\n"
                
                if channels:
                    report_text += f"📢 <b>Каналы ({len(channels)}):</b>\n"
                    for i, chat in enumerate(channels[:10], 1):
                        username_str = f" @{chat['username']}" if chat.get('username') else ""
                        report_text += f"{i}. {chat['title']}{username_str}\n"
                    
                    if len(channels) > 10:
                        report_text += f"... и еще {len(channels) - 10} каналов\n"
                    report_text += "\n"
                
                if private:
                    report_text += f"👤 <b>Личные чаты ({len(private)}):</b>\n"
                    for i, chat in enumerate(private[:5], 1):
                        report_text += f"{i}. {chat['title']}\n"
                    
                    if len(private) > 5:
                        report_text += f"... и еще {len(private) - 5} личных чатов\n"
                    report_text += "\n"
                
            else:
                report_text = (
                    f"❌ <b>Чаты не найдены</b>\n\n"
                    f"👤 Пользователь: {user.first_name if hasattr(user, 'first_name') else 'ID: ' + str(user_id)}\n"
                    f"💬 Проверено диалогов: {len(dialogs)}\n\n"
                    f"<i>Пользователь не найден в доступных чатах или у вас нет доступа к информации о его участии.</i>"
                )
            
            keyboard = self.create_keyboard([
                [
                    {"text": "🔙 В меню", "callback_data": f"back_to_menu:{user_id}"},
                    {"text": "🔄 Обновить", "callback_data": f"find_user_chats:{user_id}"}
                ]
            ])
            ...
            print(f"Ошибка поиска чатов пользователя: {e}")
            await self.send_bot_message(chat_id, f"❌ Ошибка: {str(e)[:100]}")
    ...
    async def show_user_info(self, chat_id: int, user_id: int):
        ...
        try:
            ...
            # Добавляем информацию о чатах пользователя
            if profile.user_chats:
                info_text += f"💬 <b>Чаты пользователя ({len(profile.user_chats)}):</b>\n"
                
                # Группируем по типам
                groups = [c for c in profile.user_chats if c.get('is_group')]
                channels = [c for c in profile.user_chats if c.get('is_channel')]
                private = [c for c in profile.user_chats if c.get('is_user')]
                
                if groups:
                    info_text += f"👥 Группы ({len(groups)}):\n"
                    for i, chat in enumerate(groups[:5], 1):
                        chat_name = chat.get('title', f'Чат {chat.get("id", "N/A")}')[:30]
                        info_text += f"{i}. {chat_name}\n"
                    if len(groups) > 5:
                        info_text += f"... и еще {len(groups) - 5} групп\n"
                    info_text += "\n"
                
                if channels:
                    info_text += f"📢 Каналы ({len(channels)}):\n"
                    for i, chat in enumerate(channels[:5], 1):
                        chat_name = chat.get('title', f'Чат {chat.get("id", "N/A")}')[:30]
                        info_text += f"{i}. {chat_name}\n"
                    if len(channels) > 5:
                        info_text += f"... и еще {len(channels) - 5} каналов\n"
                    info_text += "\n"
                
                if private:
                    info_text += f"👤 Личные чаты ({len(private)}):\n"
                    for i, chat in enumerate(private[:3], 1):
                        chat_name = chat.get('title', f'Чат {chat.get("id", "N/A")}')[:30]
                        info_text += f"{i}. {chat_name}\n"
                    if len(private) > 3:
                        info_text += f"... и еще {len(private) - 3} личных чатов\n"
                    info_text += "\n"
            ...
            keyboard = self.create_keyboard([
                [
                    {"text": "💬 Найти все чаты", "callback_data": f"find_user_chats:{user_id}"},
    ...
    async def search_user_messages(self, chat_id: int, user_id: int, search_text: str):
        ...
        try:
            ...
            for i, chat_identifier in enumerate(chats, 1):
                try:
                    if i % 3 == 0:  # Прогресс каждые 3 чата
                        progress_msg = (
                            f"🔍 Прогресс: {i}/{len(chats)} чатов\n"
    ...
    async def search_replies_to_user(self, chat_id: int, user_id: int, target_user_input: str):
        ...
        try:
            ...
            for i, chat_identifier in enumerate(chats, 1):
                try:
                    if i % 2 == 0:  # Прогресс каждые 2 чата
                        progress_msg = (
                            f"🔍 Прогресс: {i}/{len(chats)} чатов\n"
                            ...
                            async for reply in self.client.iter_messages(
                                ...
                                if (reply and reply.reply_to and 
                                    ...
                                    try:
                                        ...
                                        continue
                                    break  # Нашли один ответ, можно перейти к следующему сообщению
    ...
    async def run(self):
        ...
        print("="*60)
        print("✨ Новая функция: Поиск чатов пользователя")
        print("="*60)
        ...
        test_msg = (
            ...
            f"✨ <b>Новые функции:</b>\n"
            f"• Поиск чатов, в которых состоит пользователь\n"
            f"• Группировка по типам чатов\n"
            f"• Сохранение информации о чатах\n"
            f"• Автоматический поиск при добавлении пользователя\n\n"
