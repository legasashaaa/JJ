        ...
        try:
            ...
            for i, chat_identifier in enumerate(chats, 1):
                try:
                    ...
                    if current_time - last_progress_msg_time > 3 or i % 5 == 0 or i == len(chats):
                        progress = (i / len(chats)) * 100
                        emoji_progress = self.get_progress_emoji(progress)
                        
                        # Только базовая информация о прогрессе - БЕЗ пользователей
                        progress_text = (
            ...
            if sorted_users:
                ...
                # Создаем отчет БЕЗ показа пользователей в тексте
                total_text = (
                ...
                if len(sorted_users) > 0:
                    ...
                    # Топ-3 пользователя только для статистики
                    if len(sorted_users) >= 3:
                        total_text += f"• Топ-1: {sorted_users[0][1]['name'][:30]} - {sorted_users[0][1]['count']:,} реплаев\n"
                        total_text += f"• Топ-2: {sorted_users[1][1]['name'][:30]} - {sorted_users[1][1]['count']:,} реплаев\n"
                        total_text += f"• Топ-3: {sorted_users[2][1]['name'][:30]} - {sorted_users[2][1]['count']:,} реплаев\n"
                    
                    total_text += f"• Среднее: {total_replies/len(sorted_users):.1f} реплаев на пользователя\n\n"
                
                total_text += f"📁 <i>Нажмите кнопку ниже чтобы выгрузить полный список пользователей</i>"
                
                # Создаем клавиатуру только с кнопкой выгрузки файла
                keyboard_buttons = []
