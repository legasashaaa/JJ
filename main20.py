...
class TelegramSpyBot:
    def __init__(self):
        ...
        self.search_results_cache: Dict[int, Dict[str, List]] = {}  # Кэш для результатов поиска {user_id: {search_text: results}}
        self.load_config()
    ...
    async def handle_callback_query(self, update: Dict):
        ...
        try:
            ...
            if action == "user_info":
                ...
                user_id = int(parts[1])
                search_text = parts[2]
                await self.export_search_results_to_file(chat_id, user_id, search_text)
    ...
    async def ask_search_text(self, chat_id: int, user_id: int):
        ...
        await self.send_bot_message(chat_id,
            ...
            f"🔎 Я найду все сообщения с этим текстом и сохраню в файл."
        )
    ...
    async def search_user_messages(self, chat_id: int, user_id: int, search_text: str):
        """Ищет сообщения пользователя (без лимитов) и сохраняет в файл"""
        try:
            await self.send_bot_message(chat_id, 
                f"🔎 Начинаю поиск сообщений с текстом: '{search_text}'\n"
                f"👤 Пользователь ID: <code>{user_id}</code>\n"
                f"📁 Все найденные сообщения будут сохранены в файл"
            )
            ...
            cache_key = f"{user_id}:{search_text}"
            self.search_results_cache[cache_key] = found_messages
            
            # Формируем и отправляем файл
            if found_messages:
                # Создаем файл со всеми результатами
                filename = f"search_results_{user_id}_{search_text[:20]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                file_content = ""
                ...
                file_content += "=" * 80 + "\n"
                file_content += f"👤 Пользователь: ID {user_id}\n"
                file_content += f"🔍 Текст поиска: '{search_text}'\n"
                file_content += f"📅 Дата поиска: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}\n"
                file_content += f"📊 Найдено сообщений: {len(found_messages)}\n"
                file_content += f"📁 Всего чатов в списке: {len(chats)}\n"
                file_content += f"✅ Проверено чатов: {checked_chats}\n"
                file_content += f"⏱ Время выполнения: {time.time() - start_time:.1f} сек\n"
                file_content += "=" * 80 + "\n\n"
                
                # Добавляем все сообщения
                for j, msg in enumerate(found_messages, 1):
                    file_content += f"СООБЩЕНИЕ #{j}:\n"
                ...
                caption = (
                    ...
                    f"✅ Проверено чатов: {checked_chats}\n"
                    f"⏱ Время выполнения: {time.time() - start_time:.1f} сек\n\n"
                    f"📄 <b>Все результаты сохранены в файле</b>"
                )
                
                await self.send_document(chat_id, filename, file_content.encode('utf-8'), caption)
                
                # Добавляем кнопку для повторной выгрузки
                keyboard = self.create_keyboard([
                    [
                        {"text": "📁 Выгрузить файл еще раз", "callback_data": f"export_search_results:{user_id}:{search_text[:50]}"},
                ...
                await self.send_bot_message(chat_id, 
                    ...
                    f"💾 Сохранено сообщений: {len(found_messages):,}\n"
    ...
    async def export_search_results_to_file(self, chat_id: int, user_id: int, search_text: str):
        """Выгружает результаты поиска в файл"""
        try:
            cache_key = f"{user_id}:{search_text}"
            if cache_key not in self.search_results_cache:
                await self.send_bot_message(chat_id, "❌ Результаты поиска не найдены в кэше")
                return
            
            found_messages = self.search_results_cache[cache_key]
            
            if not found_messages:
                await self.send_bot_message(chat_id, "❌ Нет результатов для выгрузки")
                return
            
            await self.send_bot_message(chat_id, "📁 Формирую файл с результатами поиска...")
            ...
            file_content += " " * 25 + "РЕЗУЛЬТАТЫ ПОИСКА СООБЩЕНИЙ\n"
            file_content += "=" * 80 + "\n"
            file_content += f"👤 Пользователь: {user_name} (ID: {user_id})\n"
            file_content += f"🔍 Текст поиска: '{search_text}'\n"
            file_content += f"📅 Дата поиска: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}\n"
            file_content += f"📊 Найдено сообщений: {len(found_messages)}\n"
            file_content += "=" * 80 + "\n\n"
            ...
            for msg in found_messages:
                chat_name = msg['chat']
                if chat_name not in chats_dict:
                    chats_dict[chat_name] = []
                chats_dict[chat_name].append(msg)
            
            # Добавляем сообщения по чатам
            for chat_name, messages in chats_dict.items():
                file_content += f"💬 ЧАТ: {chat_name}\n"
                file_content += f"📊 Сообщений в этом чате: {len(messages)}\n"
                file_content += "-" * 60 + "\n"
                
                for j, msg in enumerate(messages, 1):
                    file_content += f"#{j} - {msg['date']}\n"
                    file_content += f"🔗 {msg['link']}\n"
                    file_content += f"📝 {msg['text'][:200]}{'...' if len(msg['text']) > 200 else ''}\n"
                    file_content += "-" * 40 + "\n"
            ...
            # Статистика
            file_content += "=" * 80 + "\n"
            file_content += " " * 30 + "СТАТИСТИКА\n"
            file_content += "=" * 80 + "\n"
            file_content += f"Всего чатов с результатами: {len(chats_dict)}\n"
            file_content += f"Всего сообщений: {len(found_messages)}\n"
            file_content += f"Среднее на чат: {len(found_messages)/max(len(chats_dict), 1):.1f}\n"
            
            # Топ чатов
            file_content += "\n🏆 Топ чатов по количеству сообщений:\n"
            sorted_chats = sorted(chats_dict.items(), key=lambda x: len(x[1]), reverse=True)
            for i, (chat_name, messages) in enumerate(sorted_chats[:10], 1):
                file_content += f"{i}. {chat_name[:40]}: {len(messages)} сообщ.\n"
            
            file_content += "\n" + "=" * 80 + "\n"
            file_content += " " * 30 + "КОНЕЦ ОТЧЕТА\n"
            file_content += "=" * 80 + "\n"
            
            caption = (
                f"📄 <b>Файл с результатами поиска</b>\n\n"
                f"👤 Пользователь: {user_name}\n"
                f"🔍 Текст: '{search_text}'\n"
                f"📊 Сообщений: {len(found_messages):,}\n"
                f"💬 Чатов: {len(chats_dict)}\n"
            ...
            print(f"Ошибка выгрузки результатов поиска: {e}")
            await self.send_bot_message(chat_id, f"❌ Ошибка: {str(e)[:100]}")
    ...
    async def ask_target_user_for_replies(self, chat_id: int, user_id: int):
        ...
        await self.send_bot_message(chat_id,
            ...
            f"• Все реплаи будут сохранены в файл\n\n"
    ...
    async def search_replies_to_specific_user(self, chat_id: int, user_id: int, target_user_input: str):
        """Ищет реплаи нашего пользователя конкретному пользователю (без лимитов) и сохраняет в файл"""
        try:
            await self.send_bot_message(chat_id, 
                f"🔍 Начинаю поиск реплаев нашего пользователя пользователю '{target_user_input}'...\n"
                f"👤 Наш пользователь ID: <code>{user_id}</code>\n"
                f"📁 Все найденные реплаи будут сохранены в файл"
            )
                                    ...
                                    if original_msg and hasattr(original_msg, 'from_id') and original_msg.from_id:
                                        ...
                                        if original_sender.id == target_user.id:
                                            ...
                                            found_replies.append({
                                                "chat": chat_name,
                                                "original_text": original_msg.text if original_msg.text else "без текста",
                                                "reply_text": message.text if message.text else "без текста",
            ...
            # Формируем и отправляем файл со всеми результатами
            if found_replies:
                ...
                # Создаем файл со всеми реплаями
                filename = f"replies_to_{target_user.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                file_content = ""
                
                # Заголовок файла
                file_content += "=" * 80 + "\n"
                file_content += " " * 25 + "РЕПЛАИ ПОЛЬЗОВАТЕЛЯ\n"
                file_content += "=" * 80 + "\n"
                file_content += f"👤 Наш пользователь: ID {user_id}\n"
                file_content += f"👥 Целевой пользователь: {target_name} (ID: {target_user.id})\n"
                ...
                for reply in found_replies:
                    ...
                    if chat_name not in chats_dict:
                        chats_dict[chat_name] = []
                    chats_dict[chat_name].append(reply)
                
                # Добавляем все реплаи
                for chat_name, replies in chats_dict.items():
                    file_content += f"💬 ЧАТ: {chat_name}\n"
                    file_content += f"📊 Реплаев в этом чате: {len(replies)}\n"
                    file_content += "-" * 60 + "\n"
                    
                    for j, reply in enumerate(replies, 1):
                        file_content += f"РЕПЛАЙ #{j}:\n"
                        file_content += f"📅 Дата реплая: {reply['reply_time']}\n"
                        file_content += f"🔗 Реплай: {reply['reply_link']}\n"
                        file_content += f"🔗 Оригинал: {reply['original_link']}\n"
                        file_content += f"📝 Оригинальное сообщение:\n{reply['original_text'][:500]}{'...' if len(reply['original_text']) > 500 else ''}\n"
                        file_content += f"📝 Ответ нашего пользователя:\n{reply['reply_text'][:500]}{'...' if len(reply['reply_text']) > 500 else ''}\n"
                        file_content += "-" * 40 + "\n\n"
                ...
                file_content += " " * 30 + "СТАТИСТИКА\n"
                file_content += "=" * 80 + "\n"
                file_content += f"Всего чатов с реплаями: {len(chats_dict)}\n"
                file_content += f"Всего реплаев: {len(found_replies)}\n"
                
                # Топ чатов
                file_content += "\n🏆 Топ чатов по количеству реплаев:\n"
                sorted_chats = sorted(chats_dict.items(), key=lambda x: len(x[1]), reverse=True)
                for i, (chat_name, replies) in enumerate(sorted_chats[:10], 1):
                    file_content += f"{i}. {chat_name[:40]}: {len(replies)} реплаев\n"
                ...
                caption = (
                    ...
                    f"✅ Проверено чатов: {checked_chats}\n"
                    f"⏱ Время выполнения: {time.time() - start_time:.1f} сек\n\n"
                    f"📄 <b>Все реплаи сохранены в файле</b>"
                )
                ...
                await self.send_bot_message(chat_id, 
                    ...
                    f"💾 Сохранено реплаев: {len(found_replies):,}\n"
                    f"📋 Используйте кнопки для управления",
                    keyboard
                )
            else:
    ...
    async def run(self):
        ...
        print("• 🔍 Поиск сообщений по тексту")
        print("• 📈 Все результаты сохраняются в файл")
        print("• 📁 Выгрузка всех результатов в текстовый файл")
        print("• ⏱️ Нет промежуточных сообщений - только финальный файл")
        print("="*60)
        ...
        test_msg = (
            f"🤖 <b>Шпионский бот запущен! Версия 3.8</b>\n\n"
            ...
            f"✨ <b>Новая система отчетов:</b>\n"
            f"• 🔍 Поиск реплаев → файл со всеми реплаями\n"
            f"• 🔍 Поиск сообщений → файл со всеми сообщениями\n"
            f"• 📊 Анализ всех реплаев → детальный файл\n"
            f"• ⏱️ Нет промежуточных сообщений\n"
            f"• 📁 Только финальный файл с результатами\n\n"
