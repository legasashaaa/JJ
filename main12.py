...
class TelegramSpyBot:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    # Последняя попытка - через get_entity с id
                                    user = await self.client.get_entity(int(user_id))
                                except Exception as e:
                                    print(f"Все методы поиска по ID не удались: {e}")
                                    await self.send_bot_message(chat_id, f"❌ Пользователь с ID {user_id} не найден")
                                    return
                except Exception as e:
                    print(f"Ошибка поиска по ID: {e}")
                    await self.send_bot_message(chat_id, f"❌ Ошибка поиска пользователя: {str(e)[:100]}")
                    return
            else:
                # Поиск по username
                ...
                try:
                    ...
                    try:
                        async for message in self.client.iter_messages(
                            chat,
                            from_user=user  # Без лимита - получаем все сообщения
                        ):
                            if message:
                                message_count += 1
                    except:
    ...
    async def search_user_messages(self, chat_id: int, user_id: int, search_text: str):
        ...
        try:
            await self.send_bot_message(chat_id, 
                ...
                f"👤 Пользователь: <code>{user_id}</code>\n"
                f"📝 Текст: '{search_text}'\n\n"
                f"<i>Поиск может занять некоторое время...</i>"
            )
            ...
            for i, chat_identifier in enumerate(chats, 1):
                try:
                    ...
                    # Ищем ВСЕ сообщения пользователя (без лимита)
                    async for message in self.client.iter_messages(
                        chat,
                        from_user=user  # Без лимита - получаем все сообщения
                    ):
        ...
        try:
            ...
            for i, chat_identifier in enumerate(chats, 1):
                try:
                    ...
                    # Получаем ВСЕ сообщения нашего пользователя в этом чате (без лимита)
                    user_messages = []
                    async for message in self.client.iter_messages(
                        chat,
                        from_user=user  # Без лимита
                    ):
                        if message:
                    ...
                    for user_msg in user_messages:
                        try:
                            ...
                            # Используем iter_messages без лимита для поиска реплаев
    ...
    async def get_chat_by_identifier(self, identifier: str):
        """Получает чат по идентификатору (поддерживает ссылки с + и обычные)"""
        try:
            identifier = identifier.strip()
            
            # Проверяем, является ли это ссылкой t.me/+
            if identifier.startswith("https://t.me/+") or identifier.startswith("t.me/+"):
                # Это ссылка с плюсом, извлекаем код
                if identifier.startswith("https://"):
                    identifier = identifier[13:]  # Убираем "https://t.me/+"
                else:
                    identifier = identifier[7:]   # Убираем "t.me/+"
                
                # Теперь identifier это что-то вроде "tmE98W5NO6xlYmQy"
                try:
                    # Пробуем получить чат по invite ссылке
            ...
            # Если это обычная ссылка с @
            elif identifier.startswith("https://t.me/") or identifier.startswith("t.me/"):
                if identifier.startswith("https://"):
                    identifier = identifier[13:]  # Убираем "https://t.me/"
                else:
                    identifier = identifier[7:]   # Убираем "t.me/"
            ...
            # Проверяем, является ли это ID канала/супергруппы
            if identifier.startswith('-100') and identifier[4:].isdigit():
                ...
                try:
                    ...
                    # Пробуем как PeerChannel если это отрицательный ID
                    if chat_id < 0:
                        try:
                            return await self.client.get_entity(PeerChannel(chat_id))
                        except:
    ...
    async def run(self):
        ...
        print("="*60)
        print("✨ НОВАЯ ВЕРСИЯ:")
        print("• Убраны все лимиты на поиск сообщений и реплаев")
        print("• Поддержка ссылок вида https://t.me/+tmE98W5NO6xlYmQy")
        print("• Улучшен поиск пользователей по ID")
        print("• Поиск ВСЕХ сообщений без ограничений")
        print("="*60)
        ...
        test_msg = (
            f"🤖 <b>Шпионский бот запущен! (v4.0)</b>\n\n"
            ...
            f"✨ <b>ГЛАВНЫЕ УЛУЧШЕНИЯ:</b>\n"
            f"• Убраны ВСЕ лимиты на поиск сообщений\n"
            f"• Поддержка ссылок с + (приватные чаты)\n"
            f"• Улучшен поиск пользователей по ID\n"
            f"• Ищет ВСЕ сообщения без ограничений\n"
            f"• Ищет ВСЕ реплаи без ограничений\n\n"
