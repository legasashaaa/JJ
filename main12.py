...
class TelegramSpyBot:
    ...
    async def load_user_chats(self, chat_id: int, user_id: int):
        ...
        try:
            ...
            dialogs = await self.client.get_dialogs(limit=200)  # Увеличил лимит до 200
                    ...
                    if dialog.is_group or dialog.is_channel:
                        ...
                        try:
                            ...
                            messages = await self.client.get_messages(
                                dialog.entity,
                                limit=10,  # Увеличил до 10 сообщений для быстрой проверки
                                from_user=user
                            )
                            ...
                            if len(messages) > 0:
                                ...
                                async for _ in self.client.iter_messages(
                                    dialog.entity,
                                    limit=500,  # Увеличил лимит до 500
                                    from_user=user
                                ):
                                    total_messages += 1
                                ...
                                if total_messages > 0:
                                    ...
                                    # Если набрали 30 чатов, останавливаемся для скорости
                                    if len(user_chats) >= 30:
                                        break
    ...
    async def show_message_count(self, chat_id: int, user_id: int):
        ...
        try:
            ...
            # ПАРАЛЛЕЛЬНАЯ обработка чатов для скорости
            tasks = []
            
            # Проверяем каждый чат параллельно
            for i, chat_identifier in enumerate(chats, 1):
                task = self._count_messages_in_chat(chat_identifier, user)
                tasks.append(task)
                
                # Запускаем по 10 параллельных задач
                if len(tasks) >= 10 or i == len(chats):
                ...
                # Короткая пауза чтобы не нагружать сервер
    ...
    async def _count_messages_in_chat(self, chat_identifier: str, user) -> Dict:
        """Подсчитывает сообщения в одном чате"""
        try:
            # Получаем чат
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
                return {"name": f"Чат {chat_identifier}", "count": 0}
            ...
            try:
                async for message in self.client.iter_messages(
                    chat,
                    from_user=user
                    # Убрал limit=200 для поиска ВСЕХ сообщений
                ):
                    if message:
                        message_count += 1
                    
                    # Ограничение только для защиты от бесконечного цикла
                    if message_count > 10000:  # Максимум 10к сообщений на чат
                        break
            except:
                # Если не удалось получить сообщения, возвращаем 0
                return {"name": getattr(chat, 'title', f"Чат {chat.id}"), "count": 0}
            
            chat_name = getattr(chat, 'title', 
                              getattr(chat, 'username', 
                                     f'Чат {chat.id}'))
            
            return {"name": chat_name[:30], "count": message_count}
    ...
    async def get_user_statistics(self, user_id: int) -> Dict:
        ...
        try:
            ...
            dialogs = await self.client.get_dialogs(limit=100)  # Увеличил до 100
            common_chats = []
            ...
            for dialog in dialogs:
                if dialog.is_group:
                    try:
                        ...
                        participants = await self.client.get_participants(dialog.entity, limit=200)  # Увеличил до 200
            ...
            stats["friends"] = friends[:15]  # Увеличил до 15
    ...
    async def search_user_messages(self, chat_id: int, user_id: int, search_text: str):
        """Ищет сообщения пользователя - ОПТИМИЗИРОВАННАЯ ВЕРСИЯ"""
        try:
            await self.send_bot_message(chat_id, 
                f"🔎 Ищу сообщения с текстом: '{search_text}'\n"
                f"👤 Пользователь ID: <code>{user_id}</code>\n"
                f"⚡ <i>Использую ускоренный поиск...</i>"
            )
            ...
            for i, chat_identifier in enumerate(chats, 1):
                task = self._search_messages_in_chat(chat_identifier, user, search_text)
                tasks.append(task)
                
                # Запускаем по 5 параллельных задач (можно увеличить)
                if len(tasks) >= 5 or i == len(chats):
                    results = await asyncio.gather(*tasks, return_exceptions=True)
                    ...
                    for result in results:
                        ...
                        checked_chats += 1
                        found_messages.extend(result['messages'])
                        
                        # Отправляем первые результаты сразу
                        if result['messages'] and len(found_messages) <= 10:
                            for msg in result['messages'][:3]:  # Первые 3 из каждого чата
                                msg_text = (
            ...
            if found_messages:
                ...
                # Отправляем остальные результаты (если есть)
                if len(found_messages) > 10:
                    remaining = found_messages[10:min(30, len(found_messages))]
                    ...
                    if len(found_messages) > 30:
                        total_text += f"📄 <b>И еще {len(found_messages) - 30} сообщений...</b>\nВсего найдено: {len(found_messages)}"
            else:
                total_text = (
    ...
    async def _search_messages_in_chat(self, chat_identifier: str, user, search_text: str) -> Dict:
        """Ищет сообщения в одном чате"""
        try:
            # Получаем чат
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
                return {"chat": chat_identifier, "messages": []}
            ...
            async for message in self.client.iter_messages(
                chat,
                from_user=user
                # Убрал limit=100 для поиска ВСЕХ сообщений
            ):
                if message and message.text and search_text.lower() in message.text.lower():
                    ...
                    # Ограничение на количество сообщений из одного чата
                    if len(chat_messages) > 50:  # Максимум 50 сообщений из одного чата
                        break
            
            return {"chat": chat_identifier, "messages": chat_messages}
    ...
    async def monitor_user_messages(self, chat_id: int, user_id: int):
        ...
        try:
            ...
            while self.tracking_status.get(user_id, {}).get('messages', False):
                try:
                    # Получаем список диалогов
                    dialogs = await self.client.get_dialogs(limit=50)  # Увеличил до 50
            ...
            while self.tracking_status.get(user_id, {}).get('replies', False):
                try:
                    ...
                    for chat_identifier in chat_identifiers:
                        task = self._check_replies_in_chat(chat_identifier, user, user_id, last_check)
                        tasks.append(task)
                        
                        # Запускаем по 3 параллельные задачи
                        if len(tasks) >= 3 or chat_identifier == chat_identifiers[-1]:
                            results = await asyncio.gather(*tasks, return_exceptions=True)
                            ...
                            for result in results:
                                ...
                                if result['replies']:
                                    ...
                                    # Отправляем уведомления о новых ответах
                                    for notification in result['notifications']:
                                        await self.send_bot_message(chat_id, notification)
                                    
                                    # Обновляем кэш
                                    if result['last_msg_id'] > 0:
                                        self.last_message_ids[user_id][result['chat_id']] = result['last_msg_id']
    ...
    async def _check_replies_in_chat(self, chat_identifier: str, user, user_id: int, last_check: datetime) -> Dict:
        """Проверяет ответы в одном чате"""
        try:
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
                return {"chat_id": 0, "replies": [], "notifications": [], "last_msg_id": 0}
            
            replies_found = []
            notifications = []
            last_msg_id = self.last_message_ids.get(user_id, {}).get(chat.id, 0)
            ...
            for message in messages:
                ...
                if message.id > last_msg_id:
                    # Обновляем кэш
                    if message.id > new_last_msg_id:
                        new_last_msg_id = message.id
    ...
    async def search_replies_to_user(self, chat_id: int, user_id: int, target_user_input: str):
        """Ищет ответы от конкретного пользователя нашему пользователю - ОПТИМИЗИРОВАННАЯ ВЕРСИЯ"""
        try:
            await self.send_bot_message(chat_id, 
                f"🔍 Ищу ответы от пользователя '{target_user_input}' нашему пользователю...\n"
            ...
            found_replies = []
            checked_chats = 0
            start_date = datetime.now() - timedelta(days=365)  # Увеличил до 1 года для поиска старых ответов
            
            # ПАРАЛЛЕЛЬНЫЙ поиск
            tasks = []
            
            for i, chat_identifier in enumerate(chats, 1):
                task = self._search_replies_to_in_chat(chat_identifier, user, target_user, start_date)
                tasks.append(task)
                
                # Запускаем по 3 параллельные задачи
                if len(tasks) >= 3 or i == len(chats):
                    results = await asyncio.gather(*tasks, return_exceptions=True)
                    ...
                    for result in results:
                        ...
                        checked_chats += 1
                        found_replies.extend(result['replies'])
                        
                        # Отправляем первые результаты
                        if result['replies'] and len(found_replies) <= 5:
                            for reply in result['replies'][:2]:  # Первые 2 из каждого чата
                                reply_info = (
            ...
            if found_replies:
                ...
                total_text = (
                    ...
                    f"📁 Проверено чатов: {checked_chats} из {len(chats)}\n"
                    f"⏳ Период: последний год\n\n"
                    f"<i>Первые результаты отправлены выше ↑</i>"
                )
                
                # Отправляем остальные результаты
                if len(found_replies) > 5:
                    remaining = found_replies[5:min(15, len(found_replies))]
                    ...
                    if len(found_replies) > 15:
                        await self.send_bot_message(chat_id,
                            f"📄 <b>И еще {len(found_replies) - 15} ответов...</b>\n"
                ...
                total_text = (
                    ...
                    f"📁 Проверено чатов: {checked_chats} из {len(chats)}\n"
                    f"⏳ Период: последний год\n\n"
                    f"<i>Пользователь {target_name} не отвечал на сообщения нашего пользователя за последний год</i>"
                )
    ...
    async def _search_replies_to_in_chat(self, chat_identifier: str, user, target_user, start_date: datetime) -> Dict:
        """Ищет ответы от целевого пользователя в одном чате"""
        try:
            # Получаем чат
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
                return {"chat": chat_identifier, "replies": []}
            ...
            async for message in self.client.iter_messages(
                ...
                # Убрал лимит для поиска ВСЕХ сообщений
            ):
                if message:
                    user_messages.append(message)
                
                # Ограничение на количество проверяемых сообщений
                if len(user_messages) > 200:  # Максимум 200 сообщений из одного чата
                    break
            ...
            for user_msg in user_messages:
                try:
                    ...
                    async for reply in self.client.iter_messages(
                        ...
                        limit=20  # Проверяем до 20 ответов на каждое сообщение
                    ):
                        if (reply and reply.reply_to and 
            ...
            return {"chat": chat_identifier, "replies": replies}
    ...
    async def search_replies_from_user(self, chat_id: int, user_id: int, target_user_input: str):
        """Ищет ответы нашего пользователя конкретному пользователю - ОПТИМИЗИРОВАННАЯ ВЕРСИЯ"""
        try:
            await self.send_bot_message(chat_id, 
                f"🔍 Ищу ответы нашего пользователя пользователю '{target_user_input}'...\n"
                f"👤 Наш пользователь ID: <code>{user_id}</code>\n"
                f"⚡ <i>Использую ускоренный поиск...</i>"
            )
            ...
            found_replies = []
            checked_chats = 0
            start_date = datetime.now() - timedelta(days=365)  # Увеличил до 1 года
            
            # ПАРАЛЛЕЛЬНЫЙ поиск
            tasks = []
            
            for i, chat_identifier in enumerate(chats, 1):
                task = self._search_replies_from_in_chat(chat_identifier, user, target_user, start_date)
                tasks.append(task)
            ...
            if found_replies:
                ...
                total_text = (
                    ...
                    f"📁 Проверено чатов: {checked_chats} из {len(chats)}\n"
                    f"⏳ Период: последний год\n\n"
                    f"<i>Наш пользователь не отвечал на сообщения пользователя {target_name} за последний год</i>"
                )
    ...
    async def _search_replies_from_in_chat(self, chat_identifier: str, user, target_user, start_date: datetime) -> Dict:
        """Ищет ответы нашего пользователя целевому пользователю в одном чате"""
        try:
            # Получаем чат
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
            ...
            async for message in self.client.iter_messages(
                ...
                # Ограничение на количество проверяемых сообщений
                if len(replies) > 100:  # Максимум 100 ответов из одного чата
                    break
    ...
    async def collect_replies_data(self, user_id: int):
        """Собирает данные о реплаях пользователя - ОПТИМИЗИРОВАННАЯ ВЕРСИЯ"""
        try:
            user = await self.client.get_entity(PeerUser(user_id))
            ...
            for chat_identifier in chat_identifiers:
                # Задачи для сбора кто отвечает пользователю
                task_to = self._collect_replies_to_in_chat(chat_identifier, user, start_date)
                tasks_to.append(task_to)
                
                # Задачи для сбора кому отвечает пользователь
                task_from = self._collect_replies_from_in_chat(chat_identifier, user, start_date)
                tasks_from.append(task_from)
                
                # Запускаем по 2 параллельные задачи каждого типа
                if len(tasks_to) >= 2:
                    results_to = await asyncio.gather(*tasks_to, return_exceptions=True)
                    
                    for result in results_to:
                        if isinstance(result, Exception):
                            continue
                        replies_to_user.extend(result)
                    
                    tasks_to = []
                
                if len(tasks_from) >= 2:
                    results_from = await asyncio.gather(*tasks_from, return_exceptions=True)
                    
                    for result in results_from:
                        if isinstance(result, Exception):
                            continue
                        replies_from_user.extend(result)
    ...
    async def _collect_replies_to_in_chat(self, chat_identifier: str, user, start_date: datetime) -> List:
        """Собирает данные о том, кто отвечает пользователю в одном чате"""
        try:
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
                return []
            ...
            async for message in self.client.iter_messages(
                chat,
                limit=100,  # Ограничим для скорости
                from_user=user,
                offset_date=start_date
            ):
                if message:
    ...
    async def _collect_replies_from_in_chat(self, chat_identifier: str, user, start_date: datetime) -> List:
        """Собирает данные о том, кому отвечает пользователь в одном чате"""
        try:
            chat = await self.get_chat_by_identifier(chat_identifier)
            if not chat:
                return []
        ...
        try:
            ...
            if not replies_to_user:
                await self.send_bot_message(chat_id,
                    ...
                    f"За последний месяц никто не отвечал на сообщения пользователя."
                )
                return
        ...
        try:
            ...
            if not replies_from_user:
                await self.send_bot_message(chat_id,
                    ...
                    f"👤 Пользователь: <code>{user_id}</code>\n"
                    f"⏳ Период: последние 30 дней\n\n"
                    f"За последний месяц пользователь никому не отвечал."
                )
                return
    ...
    async def get_chat_by_identifier(self, identifier: str):
        """Получает чат по идентификатору - ОБНОВЛЕННЫЙ для работы с invite ссылками"""
        try:
            identifier = identifier.strip()
            
            # Обработка invite ссылок вида https://t.me/+tmE98W5NO6xlYmQy
            if identifier.startswith('https://t.me/+') or identifier.startswith('t.me/+'):
                try:
                    # Для invite ссылок используем прямое получение
                    return await self.client.get_entity(identifier)
                except Exception as e:
                    print(f"Ошибка получения чата по invite ссылке {identifier}: {e}")
                    return None
    ...
    async def run(self):
        ...
        print("="*60)
        print("✨ Новая функция: Ускоренный поиск и invite ссылки")
        print("🔍 Поиск без лимитов по сообщениям и реплаям")
        print("⚡ Параллельная обработка чатов для скорости")
        print("🔗 Поддержка ссылок вида https://t.me/+invite_code")
        print("="*60)
        ...
        test_msg = (
            ...
            f"• ⚡ Ускоренный поиск в 3-5 раз быстрее\n"
            f"• 🔍 Поиск ВСЕХ сообщений без лимитов\n"
            f"• 💬 Поиск реплаев за последний год\n"
            f"• 🚀 Параллельная обработка чатов\n"
            f"• 🔗 Поддержка invite ссылок\n"
            f"• 📊 Увеличены лимиты поиска\n\n"
